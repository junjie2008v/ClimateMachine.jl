...
@kernel function dgsem_interface_tendency!(
    ...
    @inbounds for f in faces
        ...
        if bctag == 0
            ...
            numerical_flux_second_order_arr!(
                numerical_flux_second_order,
                balance_law,
                local_flux,
                normal_vector,
            ...
            Base.Cartesian.@nif 7 d -> bctag == d <= length(bcs) d -> begin
                ...
                numerical_boundary_flux_second_order_arr!(
                    numerical_flux_second_order,
                    bc,
                    balance_law,
                    local_flux,
    ...
    @inbounds for f in faces
        ...
        if bctag == 0  # Periodic boundary condition (boundary-less)
            ...
            Base.Cartesian.@nif 7 d -> bctag == d <= length(bcs) d -> begin
                ...
                numerical_boundary_flux_gradient_arr!(
                    numerical_flux_gradient,
                    bc,
                    balance_law,
                    local_transform_gradient,
...
@kernel function interface_divergence_of_gradients!(
    ...
    @inbounds for f in faces
        ...
        if bctag == 0
            ...
            Base.Cartesian.@nif 7 d -> bctag == d <= length(bcs) d -> begin
                bc = bcs[d]
                numerical_boundary_flux_divergence_arr!(
                    divgradnumpenalty,
                    bc,
                    balance_law,
                    l_div,
...
@kernel function interface_gradients_of_laplacians!(
    ...
    @inbounds for f in faces
        ...
        if bctag == 0
            numerical_flux_higher_order_arr!(
                hyperviscnumflux,
                balance_law,
                local_state_hyperdiffusion,
                normal_vector,
            ...
            Base.Cartesian.@nif 7 d -> bctag == d <= length(bcs) d -> begin
                bc = bcs[d]
                numerical_boundary_flux_higher_order_arr!(
                    hyperviscnumflux,
                    bc,
                    balance_law,
                    local_state_hyperdiffusion,
